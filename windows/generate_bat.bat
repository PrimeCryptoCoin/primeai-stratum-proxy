...
if NOT "%CURRENT_DIRECTORY:~-28%" == "\primeai-stratum-proxy\windows\" (
    if NOT "%CURRENT_DIRECTORY:~-35%" == "\primeai-stratum-proxy-master\windows\" (
        echo Error: Please run this batch file as-is from its original location in the primeai-stratum-proxy folder
	pause
        exit /B
    )
)
